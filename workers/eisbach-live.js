...
async function handleStatus(env) {
  if (!env?.KV) return json({ status: null, counts: {good:0,okay:0,poor:0,closed:0}, total: 0, lastVoteAt: null });
  try {
    ...
    const lastVoteAt = total > 0
      ? new Date(Math.max(...recent.map(v => v.t))).toISOString().replace(/\.\d{3}Z$/, "Z")
      : null;
    return json({ status: dominant, counts, total, lastVoteAt });
  } catch {
    return json({ status: null, counts: {good:0,okay:0,poor:0,closed:0}, total: 0, lastVoteAt: null });
  }
}
...
async function handleVote(request, env) {
  ...
  if (!VOTE_STATUSES.includes(status)) return json({ error: "invalid_status" }, 400);
  ...
  try {
    ...
    await env.KV.put(`rl:${ipHash}`, String(Date.now()), { expirationTtl: 3600 });
  } catch {
...
export default {
  async fetch(request, env) {
    ...
    if (pathname === "/wave-vote" && request.method === "POST") return handleVote(request, env);
